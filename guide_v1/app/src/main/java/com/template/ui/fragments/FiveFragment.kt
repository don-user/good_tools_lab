...
class FiveFragment : Fragment(), ArticlesAdapter.ItemClickArticle  {

    private var _binding: FragmentFiveBinding? = null
    private val binding: FragmentFiveBinding
    ...
    override fun onCreateView(
        ...
        val listArticles = ArticlesGenerator(requireContext()).getListForAdapter(Chapters.FIVE)
        setupRecyclerView(listArticles)

        backPressFragment(requireActivity() as AppCompatActivity, HOME)

        return binding.root
    }

    override fun onClickItemArticle(listNameItem: ListArticles) {
        val fragment = ArticlesFragment.newInstanceArticles(listNameItem.description, Chapters.FIVE.nameChapter)
    ...
    private fun setupRecyclerView(list: List<ListArticles>) {
        ...
        binding.recyclerViewFive.adapter = adapter
        adapter.submitList(list)
    }

    companion object{
        fun newInstanceFive() = FiveFragment()
    }
}